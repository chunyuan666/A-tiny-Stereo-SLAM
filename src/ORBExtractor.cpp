...
namespace myslam {
    ...
    ORBExtractor::ORBExtractor(int nFeatures, float scaleFactor,
        ...
        mnFeatures(nFeatures), mscaleFactor(scaleFactor),
        mnLevels(nLevels), miniThFAST(iniThFAST), mminThFAST(miniThFAST)
    {
        mvScaleFactor.resize(mnLevels);
        mvLevelSigma2.resize(mnLevels);
        mvScaleFactor[0] = 1.0f; // 金字塔地0层不缩放
    ...
    void ORBExtractor::Detect(cv::InputArray _image, cv::InputArray _mask, std::vector<cv::KeyPoint>& _keypoints){ 
        ...
        for(int i=0; i<nRows; i++)
        {
            ...
            for(int j=0; j<nCols; j++)
            {
                ...
                FAST(image.rowRange(iniY,maxY).colRange(iniX,maxX),
                        vKeysCell,miniThFAST,true);

                if(vKeysCell.empty())
                {
                    FAST(image.rowRange(iniY,maxY).colRange(iniX,maxX),
                            vKeysCell,mminThFAST,true);
                }
    ...
    void ORBExtractor::CalcuKeyPoints(cv::InputArray img, std::vector<cv::KeyPoint> &kps, 
        std::vector<cv::KeyPoint> &outkps){
        cv::Mat image = img.getMat();
        outkps.reserve(kps.size());
        ...
        for(int i = 0; i < kps.size(); i++){
            ...
            kp.pt /= scale;
            if( !(kp.pt.y>=EDGE_THRESHOLD && kp.pt.y<imgPyramid.rows-EDGE_THRESHOLD &&
                  kp.pt.x>=EDGE_THRESHOLD && kp.pt.x<imgPyramid.cols-EDGE_THRESHOLD)){
                kp.pt *= scale;
                continue;
            }
            if(isFastCorner(imgPyramid, kp, miniThFAST)){
                kp.pt *= scale;
                continue;
            }
            kp.angle = IC_Angle(imgPyramid, kp.pt, umax);
            kp.size = PATCH_SIZE * mvScaleFactor[level];
            kp.pt *= scale;
            outkps.push_back(kp);
        }
    }

    void ORBExtractor::CalcuDescriptors(cv::Mat img, std::vector<cv::KeyPoint> &kps, cv::OutputArray descriptors){
        std::vector<cv::Mat> vBlurImgs(mnLevels);
        for(int level = 0; level < mnLevels; level++){
            cv::Mat img_blur = mvImagePyramid[level].clone();
            cv::GaussianBlur(img_blur, img_blur, cv::Size(7,7), 2, 2, cv::BORDER_REFLECT_101);
            vBlurImgs[level] = img_blur;
        }
        descriptors.create(kps.size(), 32, CV_8U);
        cv::Mat descs = descriptors.getMat();
        for(int i = 0; i < kps.size(); i++){
            cv::KeyPoint kp = kps[i];
            int level = kp.octave;
            float scale = mvScaleFactor[level];
            cv::Mat desc = descs.rowRange(i, i+1);
            desc = cv::Mat::zeros(1, 32, CV_8UC1);
            kp.pt /= scale;
            computeOrbDescriptor(kp, vBlurImgs[level], &pattern[0], desc.ptr(0));
            kp.pt *= scale;
        }
    }
